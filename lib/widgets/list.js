...
List.prototype.createItem = function(content) {
  ...
  var options = {
    ...
    tags: this.parseTags,
    height: this.style.item.height === null ? countLines(content) : (this.style.item.height || 1),
...
List.prototype.select = function(index) {
  ...
  const isDown = (index > this.selected);
  ...
  const scrollVal = (index === 0
                     ? 0
                     : (isDown
                        ? selectedItem.top + selectedItem.height
                        : selectedItem.top - 1));
  this.scrollTo(scrollVal);

  // XXX Move `action` and `select` events here.
  this.emit('select item', selectedItem, this.selected);
};
